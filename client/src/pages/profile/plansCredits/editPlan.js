...
export default function Edit() {
    ...
    const [credits, setPlanCredits] = useState('');
    ...
    const [CreditsError, setPlanCreditsError] = useState("");
    const [ErrorMessage, setErrorMessage] = useState("");


    const [oldPlanInfo, setOldPlanInfo] = useState('');
    const { id } = useParams();

    //fill in the inputs 
    useEffect(() => {
        axios.get(`/api/getCreditsPlanByID/${id}`)
            .then(response => {
                setOldPlanInfo(response.data);
                setPlanName(response.data.name);
                setPlanDescription(response.data.description);
                setPlanAmount(response.data.amount);
                setPlanCredits(response.data.credits);
            })
            .catch(error => {
    ...
    const edit = () => {
        if (validateForm()) {
            ...
            const requestData = {
                ...
                credits: credits,
                id: id,
                amount: typeof amount === 'number' ? amount * 100 : Number(String(amount).replace(/,/g, '')) * 100,
            };
    ...
    return (
        ...
        <body className=" flex justify-center h-screen mt-11">
            <div className="bg-white p-8 rounded-lg shadow-lg w-96 h-125" >
                ...
                <div>
                    <h1 className="mt-6 text-center text-3xl font-extrabold text-gray-900"> Edit {oldPlanInfo.name}</h1>
                </div>
                {ErrorMessage && (
                ...
                <div className="mt-4 rounded-md shadow-sm -space-y-px grid grid-cols-1 gap-2">
                    ...
                    <div className="items-center mt-4">
                        ...
                        />
                        <p className="text-red-500 text-xs">{CreditsError}</p>
                    </div>
